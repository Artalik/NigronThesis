...
Definition parse_packet_dh_init_rel :
  ...
  eapply exist. intros. unfold parse_packet_dh_init, combinator.map.
  repeat step. eapply parse_string_adequate.
  eapply (extern_adequate "SSHPacket" "create_DiffieHellmanInit" (CONS (Var vres) NIL)).
Defined.
...
Definition parse_packet_dh_reply_rel :
  ...
  repeat step. 1 - 3 : eapply parse_string_adequate.
...
Definition parse_packet_debug_rel :
  ...
  1-2 : eapply parse_string_adequate.
...
Definition parse_packet_disconnect_rel :
  ...
  eapply exist. intros. unfold parse_packet_disconnect. repeat step.
  eapply be_u32_adequate.
  1-2 : eapply parse_string_adequate.
...
Definition parse_packet_key_exchange_rel :
  ...
  eapply exist. unfold parse_packet_key_exchange, usize_16. intros.
  repeat step.
...
Definition parse_ssh_packet_rel :
  ...
  eapply exist. intros. unfold parse_ssh_packet. repeat step.
  eapply be_u32_adequate.
  eapply be_u8_adequate.
  be_spec_clean. repeat step.
  eapply map_parser_adequate. eapply consequence_adequate. step.
  intros. repeat clean_up. finish_me.
  intros. repeat step. eapply be_u8_adequate.
  eapply (natN_switch_adequate _ (EUna EVal (Var vres1))); be_spec_clean; finish_me.
  ...
  unfold combinator.map. step. eapply parse_string_adequate.
  ...
  unfold combinator.map. step. eapply be_u32_adequate.
  ...
  Ltac solve_default := eapply fail_adequate.
  solve_default.
  repeat (destruct elem; try (exfalso; apply elem_neq; repeat constructor; done);
          try solve_default).
  eapply (ret_adequate _ _ _ (EBin EPair (Var vres1) (Var vres2))); finish_me.
Defined.
